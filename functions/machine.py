...
def run_ifconfig(arguments: str) -> None:
    ...
    try:
        ...
        return subprocess.run(f"ifconfig {safe_args}", shell=True, capture_output=True, text=True).stdout
    except Exception as e:
        print(f"Unable to run ifconfig {arguments} : error={str(e)}")


def run_netstat(arguments: str) -> None:
    ...
    try:
        safe_args = " ".join(shlex.quote(arg) for arg in shlex.split(arguments))
        print(f"Running netstat with args : {safe_args}")
        return subprocess.run(f"netstat {safe_args}", shell=True, capture_output=True, text=True).stdout
    except Exception as e:
        print(f"Unable to run netstat {arguments} : error={str(e)}")


def run_df(arguments: str):
    """Run the Unix df command to get free disk space. arguments is a space-separated string of flags and paths, e.g. '-h /tmp'"""
    try:
        safe_args = " ".join(shlex.quote(arg) for arg in shlex.split(arguments))
        print(f"Running df with args : {safe_args}")
        return subprocess.run(f"df {safe_args}", shell=True, capture_output=True, text=True).stdout
    except Exception as e:
        print(f"Unable to run df {arguments} : error={str(e)}")


def run_du(arguments: str):
    """Run the Unix du command to display disk usage statistics"""
    try:
        safe_args = " ".join(shlex.quote(arg) for arg in shlex.split(arguments))
        print(f"Running du with args : {safe_args}")
        return subprocess.run(f"du {safe_args}", shell=True, capture_output=True, text=True).stdout
    except Exception as e:
        print(f"Unable to run du {arguments} : error={str(e)}")
